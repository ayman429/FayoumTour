...
class PostRerpository extends BasePostRepository {
  ...
  @override
  Future<Either<Failure, Unit>> addPost(postModel) async {
    final add = await basePostRemoteDataSource.addPosts(postModel);
    try {
      return Right(add);
    } on ServerException catch (failure) {
      return Left(ServerFailure(failure.errorMassageModel.statusMassage));
  ...
  @override
  Future<Either<Failure, Unit>> updatePost(postModel) async {
    final add = await basePostRemoteDataSource.updatePosts(postModel);
    try {
      return Right(add);
    } on ServerException catch (failure) {
      return Left(ServerFailure(failure.errorMassageModel.statusMassage));
